...
const AdminEditPage = () => {
  ...
  const updateMutation = useMutation({
    mutationFn: (payload: AdminPayload) => updateAdmin(adminId, payload),
    onSuccess: () => {
    ...
    onError: (error: unknown) => {
      if (isAxiosError(error)) {
        const responseErrors = (error.response?.data as { errors?: Record<string, string[] | string> } | undefined)?.errors;
        if (responseErrors) {
  ...
  const handleSubmit = async (values: AdminPayload) => {
    setErrorMessage(null);
    await updateMutation.mutateAsync(values);
  };

  const initialValues: Partial<AdminPayload> | undefined = data
    ? {
        name: data.name,
        email: data.email,
        phone: data.phone ?? "",
